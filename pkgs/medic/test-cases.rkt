...
; test match-expr
(layer layer1 
       (in #:file (file "/home/xiangqi/work/Dec/debug/factorial.rkt")
           ; insert-expr case
           [on-entry (define x 1)
                     (define y 1)]
           [on-exit (define z 1)]
           [(at (define x 1)) [on-entry (define q 1)]
                              [on-exit (define w 1)]]
           [(at (with-start |(define x|)) [on-entry (define q 1)]]
           [(at (define x 1) #:before (define e 1)) [on-entry (define t 1)]]
           [(at (define x 1) #:after (define e 1)) [on-entry (define t 1)]]
           ; each-function case
...
; multiple file in one single layer (opening unexisting file)
(layer layer5
